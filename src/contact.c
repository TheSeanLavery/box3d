									 ...
									 b3WorldTransform xfA, const b3Shape* shapeB, b3WorldTransform xfB, b3Arena arena )
{
	...
	else
	{
		...
		else
		{
			...
			taskContext->satSameHullCallCount += shapeA->hull == shapeB->hull;
			taskContext->satBoxHullCallCount += shapeA->hull->vertexCount == 8 && shapeA->hull->edgeCount == 24 &&
												shapeA->hull->faceCount == 6 && shapeB->hull->vertexCount == 8 &&
												shapeB->hull->edgeCount == 24 && shapeB->hull->faceCount == 6;
			switch ( cache->satCache.hit )
			{
				case b3_satCacheSeparationHit:
					taskContext->satCacheHitCount += 1;
					taskContext->satCacheSeparationHitCount += 1;
					break;
				case b3_satCacheFaceHit:
					taskContext->satCacheHitCount += 1;
					taskContext->satCacheFaceHitCount += 1;
					break;
				case b3_satCacheEdgeHit:
					taskContext->satCacheHitCount += 1;
					taskContext->satCacheEdgeHitCount += 1;
					break;
				default:
					taskContext->satFullSearchCount += 1;
					break;
			}
		}
	}
